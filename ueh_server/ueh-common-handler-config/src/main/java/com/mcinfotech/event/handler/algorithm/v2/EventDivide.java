...
/**
 * 事件分组
 * 如果是事件级联推送来的事件，不再进行公共事件处理策略、资源、CMDB设备属性规则，只执行定义策略
...
public class EventDivide {
	 ...
	 * @return 分组过的事件
	 */
	public static Collection<Map<String, Object>> excute(ProbeInfo probeInfo,Collection<Map<String, Object>> rawMessages, Map<String, ColumnDefine> columnDefineMap, List<EventHandlerRule> rules, DataSource dataSource){
		return excute(probeInfo,rawMessages, columnDefineMap, rules, dataSource,null);
	}
	
	public static Collection<Map<String, Object>> excute(ProbeInfo probeInfo,Collection<Map<String, Object>> rawMessages, Map<String, ColumnDefine> columnDefineMap, List<EventHandlerRule> rules, DataSource dataSource, EventSourceType eventSourceType) {
		try {
			...
			//依次从1.资源表（排除CMDB设备）、2.当前probe的策略、公共策略（没有绑定Probe）、CMDB设备进行匹配
			//如果是级联事件则执行事件处置策略，公共事件处理策略、资源、CMDB设备属性不再参与处理
			
			if(probeInfo.getEventSourceType()==EventSourceType.CASCADE) {
				for (Map<String, Object> rawMessage : rawMessages) {
					//按照当前probe的策略进行匹配
					if (CollectionUtils.isNotEmpty(probeRules)) {
						groupByRules(probeInfo,rawMessage, probeRules, columnDefineMap,dataSource);
					}
				}
			}else {
				boolean flag=false;
				for (Map<String, Object> rawMessage : rawMessages) {
					//从资源表匹配
					String node = String.valueOf(rawMessage.get(EventConstant.eventColumnNode));
					if (!"null".equals(node)) {
						DataLoadParams dataLoadParams = new DataLoadParams();
						//自定义监控
						//从资源表（t_res_resource）查询
						...
						if (rs.isSuccess()&&rs.getTotalCount()==1&& CollectionUtils.isNotEmpty(datas)) {
							...
							if (!"null".equals(name)) {
								rawMessage.put(EventConstant.eventColumnEventCategory,name);
								rawMessage.put(EventConstant.eventColumnEventCategoryLabelText,labelText);

								rawMessage.put(EventConstant.eventColumnParentEventCategory,parentName);
								rawMessage.put(EventConstant.eventColumnParentEventCategoryLabelText,parentLabelText);
								flag=true;
							}
						}
					}
					...
					if (!flag &&!CollectionUtils.isEmpty(probeRules)) {
						flag = groupByRules(probeInfo,rawMessage, probeRules, columnDefineMap,dataSource);
					}
					//当前probe的策略没有匹配上，使用公共策略进行匹配
					if (!flag && !CollectionUtils.isEmpty(rules)) {
						flag = groupByRules(probeInfo,rawMessage, rules, columnDefineMap,dataSource);
					}
					...
					if (!flag) {
						if (!EventSourceType.CUSTOM.name().equals(String.valueOf(eventSourceType))) {//自定义监控
							...
							ResultPattern rs = getGroupName(dataSource, node, dataLoadParams);
							List<Map<String, Object>> datas = rs.getDatas();
							if (rs.isSuccess()&&rs.getTotalCount()>0&& CollectionUtils.isNotEmpty(datas)) {
								String name = String.valueOf(datas.get(0).get("name"));
								String labelText = String.valueOf(datas.get(0).get("labelText"));
...
//								ResultPattern resultPattern = getGroupName1(dataSource, id, dataLoadParams);
	 ...
	 */
	private static boolean groupByRules(ProbeInfo probeInfo,Map<String, Object> rawMessage, List<EventHandlerRule> rules, Map<String, ColumnDefine> columnDefineMap,DataSource dataSource) {
		//每条策略
			for (EventHandlerRule rule : rules) {
				...
				if (isValid1) {
					rawMessage.put(EventConstant.eventColumnEventCategory,effectValue);
					rawMessage.put(EventConstant.eventColumnEventCategoryLabelText,effectType);
					rawMessage.put(EventConstant.eventColumnRefGroupRules,rule.getName());

					//新增字段补充
					DataLoadParams dataLoadParams = new DataLoadParams();
					dataLoadParams.setDcName("getParentId");
					Map<String,Object> filter = new HashMap<>();
					filter.put(EventConstant.eventColumnEventCategory,effectValue);
					ResultPattern resultPattern = getGroupNameByFilter(dataSource, filter, dataLoadParams);
					List<Map<String, Object>> data = resultPattern.getDatas();
					String parentName = "";
					String parentLabelText ="";
